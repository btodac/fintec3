...
class LSClient(object):
    ...
    def _join(self):
        ...
        if self._stream_connection_thread:
            ...
            try:
                ...
                log.debug('Unable to join stream connection thread!')
                if self._stream_connection_thread.is_alive():
                    log.debug('Stream connection thread is still alive')
                else:
                    log.debug('Stream connection thread is not alive')
                    self._stream_connection_thread = None
    ...
    def _receive(self):
        ...
        while receive and self._stream_connection_thread.active_connection:
            ...
            elif message == "": # Null string
                log.debug("Null string recieved") 
                receive = False
        ...
        if rebind:
            ...
            log.info("Closing connection: " + message)
            ...
            #raise LSError('Connection closed: ' + message)
